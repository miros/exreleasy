defmodule Exreleasy.HotReloaderTest do
  ...
  test "reloads code" do
    assert :ok == HotReloader.reload(Node.self(), "/new/path", [app_name: "0.0.1"],
  ...
  test "reloads configs" do
    assert :ok == HotReloader.reload(Node.self(), "./test_app", [app_name: "0.0.1"],
  ...
  def fake_reloader(receiver) do
    fn({app_name, _app_version}, new_path) ->
